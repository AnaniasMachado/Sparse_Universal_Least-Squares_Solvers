...
def problem_1_norm_P1_P3_P4_solver(A):
    # Extracts matrix dimensions
    m = A.shape[0]
    n = A.shape[1]
    ...
    for i in range(n):
        for j in range(n):
            model.addConstr(HA_T[i, j] == HA[i, j], name=f"constraint_P4_i={i}_j={j}")
...
def problem_1_norm_PLS_P4_solver(A):
    # Extracts matrix dimensions
    m = A.shape[0]
    n = A.shape[1]
    ...
    for i in range(n):
        for j in range(m):
            model.addConstr(ATAH[i, j] == A_T[i, j], name=f"constraint_PLS_i={i}_j={j}")
...
def problem_1_norm_PMN_P3_solver(A):
    # Extracts matrix dimensions
    m = A.shape[0]
    n = A.shape[1]
    ...
    # Adds constraint: PMN given by AA^TH^T = A
    AATHT = np.dot(A, np.dot(A.T, H.T))
    for i in range(m):
        for j in range(n):
            model.addConstr(AATHT[i, j] == A[i, j], name=f"constraint_PMN_i={i}_j={j}")
...
def problem_1_norm_PMX_solver(A):
    # Extracts matrix dimensions
    m = A.shape[0]
    n = A.shape[1]
    ...
    # Adds constraint: PMN given by AA^TH^T + H^TA^TA = 2A
    LeftHS = np.dot(A, np.dot(A.T, H.T)) + np.dot(H.T, np.dot(A.T, A))
    RightHS = 2*A
    for i in range(m):
        for j in range(n):
            model.addConstr(LeftHS[i, j] == RightHS[i, j], name=f"constraint_PMX_i={i}_j={j}")
...
def problem_1_norm_P1_sym_solver(A):
    # Extracts matrix dimensions
    m = A.shape[0]
    n = A.shape[1]
    ...
    # Adds constraint: P1 given by H^T = H
    H_T = H.T
    for i in range(n):
        for j in range(m):
            model.addConstr(H[i, j] == H_T[i, j], name=f"constraint_sym_i={i}_j={j}")
