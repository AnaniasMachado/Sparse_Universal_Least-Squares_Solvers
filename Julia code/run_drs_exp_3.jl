    ...
    if method == "DRS"
        ...
        if (m < min_DRS_unsolvable_m)
            ...
            if DRS_H == "-"
                ...
                if fixed_tol
                    solution_filename = "DRS/Experiment_$(exp)_P134_m_$(m)_n_$(n)_idx_$(idx)"
                ...
                elseif stop_crit == "Boyd"
                    solution_filename = "DRS_Boyd/Experiment_$(exp)_P134_m_$(m)_n_$(n)_idx_$(idx)"
                    solution_filepath = joinpath(solutions_folder, solution_filename)
                    matwrite(solution_filepath, Dict("H" => DRS_H, "time" => DRS_time, "k" => DRS_k))
                elseif stop_crit == "Fixed_Point"
                    solution_filename = "DRS_FP/Experiment_$(exp)_P134_m_$(m)_n_$(n)_idx_$(idx)"
                    solution_filepath = joinpath(solutions_folder, solution_filename)
                    matwrite(solution_filepath, Dict("H" => DRS_H, "time" => DRS_time, "k" => DRS_k))
                end
            end
